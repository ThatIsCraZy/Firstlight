...
func RunLauncher(ctx context.Context, initial Fields, connect func(Fields) error) error {
	...
	connectFields := func(fields Fields) {
		...
		if err := connect(fields); err != nil {
			walk.MsgBox(mw, "hpeirc login", err.Error(), walk.MsgBoxOK|walk.MsgBoxIconError)
		}
	...
	submit := func() {
		...
		if fields.Addr == "" || fields.User == "" || fields.Password == "" {
			walk.MsgBox(mw, "hpeirc login", "Adresse, Name und Passwort sind erforderlich.", walk.MsgBoxOK|walk.MsgBoxIconWarning)
			return
		}
