# Firstlight keyboard-map authoring guide
...
A valid external map whose ID is `german` completely replaces the built-in German map. If the external replacement is invalid, Firstlight reports the error and retains the built-in map. The protected `us-base` ID cannot be overridden. Two external files with the same ID are both rejected.
...
Create a complete Firstlight keyboard-map JSON file for the requested source keyboard layout by using the attached exported German JSON as the structural reference and this guide as the authoritative schema.
